...
const categories = ['全部', '专升本', 'CET4', 'CET6', 'IELTS', 'GRE', 'TOEFL', '常用'];
...
const WordsScreen: React.FC = () => {
  ...
  const playWordSound = async (word: string, id: string) => {
    try {
      if (speakingId === id) {
        Speech.stop();
        setSpeakingId(null);
      ...
      Speech.speak(word, {
        ...
        onError: () => {
          setSpeakingId(null);
          Alert.alert('播放失败', '语音播放出错，请重试');
        },
      });
    } catch (error) {
      console.error('语音播放失败:', error);
      setSpeakingId(null);
      Alert.alert('播放失败', '无法播放语音');
    }
  };

  const filteredWords = words.filter((word) => {
    const matchesCategory = selectedCategory === '全部' || word.level === selectedCategory;
  ...
  const markAsLearned = async (word: Word) => {
    if (!word.isMastered) {
      const updatedWord = { ...word, isMastered: true };
      const success = await WordStorage.updateWord(updatedWord);
      if (success) {
        addLearnedWords(1);
        loadWords(); // Reload to update UI
      }
    }
  };
  ...
  const renderWordCard = ({ item }: { item: Word }) => {
    ...
    const isLearned = item.isMastered || false;
    ...
    return (
      ...
      >
        <LinearGradient
          colors={isLearned ? [palette.grassLight, palette.grass] : [palette.surface, palette.surface]}
        ...
        >
          <View style={styles.wordHeader}>
            <View style={styles.wordTitle}>
              ...
              <Text style={styles.phoneticText}>{item.phonetic_us || item.phonetic_uk}</Text>
            </View>
            <View style={styles.wordActions}>
              ...
              <TouchableOpacity
                ...
                onPress={(e) => {
                  e.stopPropagation();
                  playWordSound(item.word, item.id);
                }}
              >
                <Ionicons
                  name={speakingId === item.id ? "volume-high" : "volume-medium"}
                  size={20}
                  color={speakingId === item.id ? palette.accent : palette.primary}
                />
              </TouchableOpacity>
            </View>
          </View>
          ...
          {isExpanded && (
            <View style={styles.wordDetails}>
              ...
              <View style={styles.detailSection}>
                ...
                <Text style={styles.exampleText}>{item.examples?.[0]?.en || '暂无例句'}</Text>
                {item.examples?.[0]?.cn && (
                  <Text style={[styles.exampleText, { marginTop: 4, color: palette.textLight }]}>
                    {item.examples[0].cn}
                  </Text>
                )}
              </View>
      ...
      <View style={styles.statsBar}>
        <View style={styles.statItem}>
          <Text style={styles.statNumber}>
            {words.filter((w) => w.isMastered).length}
          </Text>
        ...
        <View style={styles.statItem}>
          <Text style={styles.statNumber}>
            {words.length > 0 ? Math.round((words.filter((w) => w.isMastered).length / words.length) * 100) : 0}%
          </Text>
