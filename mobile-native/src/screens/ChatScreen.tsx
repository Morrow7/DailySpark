...
import { sendChatMessage, textToSpeech } from '../api/doubao';
...
const ChatScreen: React.FC = () => {
  ...
  const stopRecording = async () => {
    console.log('停止录音，当前状态:', isRecording, '时长:', recordingDuration);
  ...
  const processVoiceInput = async (audioUri: string) => {
    ...
    try {
      // 这里应该调用语音识别API
      // 由于豆包ASR需要后端支持，目前使用模拟演示
      // 实际项目中替换为真实的语音识别API调用
      ...
      // 自动发送识别的内容
      await sendMessage(randomInput);
      
    } catch (error) {
      console.error('语音识别失败:', error);
      Alert.alert('提示', '语音识别失败，请尝试打字输入');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    >
      ...
      >
        <View style={styles.headerContent}>
          <View>
            <Text style={styles.headerTitle}>AI口语练习</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyTitle}>开始口语练习</Text>
            ...
            <View style={styles.tipsContainer}>
              <Text style={styles.tipsTitle}>💡 试试说:</Text>
        ...
        {isVoiceMode ? (
          ...
          <View style={styles.voiceInputContainer}>
            ...
            >
              ...
              >
                <Ionicons name="mic" size={32} color={palette.cloud} />
              </LinearGradient>
            </TouchableOpacity>
